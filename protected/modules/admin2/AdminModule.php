...
class AdminModule extends CWebModule {
	...
	public function init() {
		...
		Yii :: app()->setComponents(array (
			...
			'cachingDuration' => 0,
			'cacheID' => 'cache',
			'basePath' => Yii :: getPathOfAlias('admin.message'
		), 'forceTranslation' => false, 'behaviors' => array (),)));
	...
	public function beforeControllerAction($controller, $action) {
		if (parent :: beforeControllerAction($controller, $action)) {
			...
			$array = array (
			'modules' => $controller->getModule()->getId(), 'controller' => $controller->id, 'action' => $action->id);

			$menus = Yii :: app()->getDb()->createCommand()->from("{{admin_role_priv}} as p")->select("id,modules,controller,action")->leftJoin("{{admin_menu}} as m", 'm.id=p.menu_id')->where('p.role_id=:rid', array (
			"rid" => self :: getUser()->role_id))->queryAll();
