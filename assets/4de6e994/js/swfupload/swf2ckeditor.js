...
function flashupload(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
	...
	window.top.art.dialog({title:name,id:uploadid,iframe:golobsetting.swfupload_url+args+setting,width:'500',height:'420'}, function(){ if(funcName) {funcName.apply(this,[uploadid,textareaid]);}else {submit_ckeditor(uploadid,textareaid);}}, function(){window.top.art.dialog({id:uploadid}).close()});
}
