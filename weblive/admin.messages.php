...
*/
	...
	while($guest = $DB->getrow($getguest)) {
		$guests[$guest['guestid']] = $guest['guestip'];
	}
	if($fromid){$searchsql="where `fromid`='$fromid' or `toid`='$fromid'";}
	...
	$maxrows = $DB->query_first("SELECT COUNT(msgid) AS value FROM " . TABLE_PREFIX . "msg ".$searchsql);
