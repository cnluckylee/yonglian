...
class CompanyXieShouController extends AdminController
{
	protected $cid;
	public function init()
	{
		...
		if($cid)
			$cid = CompanyXieShou::getCidById($cid);
		$this->cid = $cid;
	}
	/**
	 * 首页列表.
	...
	{
		$model=new CompanyXieShou('search');
		$model->unsetAttributes();  // 清理默认值
		if(isset($_GET['CompanyXieShou']))
			$model->attributes=$_GET['CompanyXieShou'];
	...
	{
		$model=new CompanyXieShou;

		// AJAX 表单验证
		$this->performAjaxValidation($model);

		if(isset($_POST['CompanyXieShou']))
		{
			$model->attributes=$_POST['CompanyXieShou'];
	...
	* @param integer $id 主键
	...
	{
		$this->redirect(array('/admin/CompanyXieShou/create','mid'=>$id));
	}
	...
	{
		$model=CompanyXieShou::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='company-xie-shou-form')
		{
