...
class CompanyTuanDuiController extends AdminController
{
	protected $cid;
	public function init()
	{
		...
		if($cid)
			$cid = CompanyTuanDui::getCidById($cid);
		$this->cid = $cid;
	}
	/**
	 * 首页列表.
	...
	{
		$model=new CompanyTuanDui('search');
		$model->unsetAttributes();  // 清理默认值
		if(isset($_GET['CompanyTuanDui']))
			$model->attributes=$_GET['CompanyTuanDui'];
	...
	{
		$model=new CompanyTuanDui;

		// AJAX 表单验证
		$this->performAjaxValidation($model);

		if(isset($_POST['CompanyTuanDui']))
		{
			$model->attributes=$_POST['CompanyTuanDui'];
	...
	* @param integer $id 主键
	...
	{
		$this->redirect(array('/admin/CompanyTuanDui/create','mid'=>$id));
	}
	...
	{
		$model=CompanyTuanDui::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='company-tuan-dui-form')
		{
