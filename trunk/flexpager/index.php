...
$(function() {
      $('table tbody tr').mouseover(function() {
		 ...
		 if(tagName != 'INPUT' && !jQuery(event.target).hasClass('title') && !jQuery(event.target).hasClass('down')){
				...
				if ($handle = opendir($configManager->getConfig('path.pdf'))) { 
					while (false !== ($file = readdir($handle))) { 
						...
						<tr class="unselectedRow">
							...
							<td class="tr" style="text-align:left;" click="1"><?php echo "$file"; ?></td>
							<td class="tr" style="text-align:left;" click="1"><?php echo filesize($configManager->getConfig('path.pdf') . $file)?>&nbsp;bytes</td>
                            <td class="tr" style="text-align:left;"><a class="down" href="<?php echo "pdf/"."$file"; ?>" target="_blank">下载pdf</a></td>
                            <td class="tr" style="text-align:left;"><a class="down" href="<?php echo "pdf/"."$file".".swf"; ?>" target="_blank">下载swf</a></td>
						</tr>
						<?php } ?>
					<?php } ?>
				<?php closedir($handle); ?>
